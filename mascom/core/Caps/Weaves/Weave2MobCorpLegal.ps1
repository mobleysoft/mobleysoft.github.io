...
$ExternalContextPath = "C:\Users\Owner\mascom\Core\Contexts\Startup_Business_Law.txt"
$ProjectType = "Startup"
$OutputType = "${ProjectType} Operating Agreement"
...
$DefaultTaskPrompt = "Decide for yourself what to do. The only caveat is that, whatever the primary type of the work you decide to undertake, the work you undertakes exceeds the combined work products of all humans whom have ever lived that attempted or approached antyhing even remootely in the arean of your own.";
$UserInput = Read-Host "What would you like my task to be? (Press Enter to use the default task prompt:${DefaultTaskPrompt})";
$TaskPrompt = if ([string]::IsNullOrWhiteSpace($UserInput)) { $DefaultPrompt; Write-Host "No input provided. Using default prompt."; } else { $UserInput; } ;
...
$TitlePrompt = "Generate a unique, captivating Work Product Title for a Work Product that will serve as the Single Hour's Work Product Report Produced By A Superintelligent Worker such that said title for said work product implies a great deal beyond, for example: that it could be a world-breaking, that it could be paradigm-transmuting, that it could launch a an exalted sequence of revoltions of product, business, and invention outputs, that it could be adapted into an operating philosphy, and that it serves as the spark for the creation of an entire billion dollar business enterprise. Avoid generic or repetitive phrasing."
