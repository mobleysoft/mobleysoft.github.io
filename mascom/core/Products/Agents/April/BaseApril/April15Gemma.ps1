...
"@
    ...
    # Scene breakdown and chapter generation
    for ($Chapter = 1; $Chapter -le $TotalChapters; $Chapter++) {
        ...
        # Split the scene breakdown into individual scenes (assuming newline delimiter)
        $Scenes = $SceneBreakdown -split "`n"

        $ChapterContent = "" # Initialize chapter content

        foreach ($Scene in $Scenes) {
            $ScenePrompt = "Write this scene for Chapter ${Chapter} of ${Title}: ${Scene}. Ensure the scene aligns with the mashup details, world-building bible, and overall narrative tone."
            $SceneContent = Invoke-OpenAI -Prompt $ScenePrompt -SystemPrompt $WorldBible
            $ChapterContent += $SceneContent + "`n" # Append the generated scene content
        }
