<# 
TICEL Specification for April5
Purpose: Generate publishable, professional-quality novels using OpenAI API.
Requirements(Requirment/Requirement Source):
1. Must Utilize TLS 1.2 for API calls. / External, Vendor, OpenAI.
#>
[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12 # Ensure TLS 1.2 for API calls
$Author = "April Carter"
...
$BasePath = "C:\Books\April5"
if (-not (Test-Path $BasePath)) { New-Item -ItemType Directory -Path $BasePath | Out-Null } # Ensure output directory exists
function Sanitize {
    param ( [string]$String )   
    $String = $String -replace '\s+', ' '  # Replace whitespace with a single space    
    $String = $String -replace '[^\w]', '' # Remove special characters (except letters, numbers, and underscores)   
...
if (-not (Test-Path $OutputFile)) { Set-Content -Path $OutputFile -Value "Generated Book Run at $(Get-Date)`n`n" } # Create output file
$ThroughputFile = Join-Path $BasePath "${SaniTitle}_SupportDoc_${RunTimestamp}.txt"
if (-not (Test-Path $ThroughputFile)) { Set-Content -Path $ThroughputFile -Value "Generated Book Run at $(Get-Date)`n`n" } # Create throughput file
...
function Write-ThroughputAndConsole {
    ...
    Add-Content -Path $ThroughputFile -Value "$Message`n"
    Write-Host $Message
}
Write-ThroughputAndConsole $TitlePrompt
function Write-OutputAndConsole {
    param ([string]$Message)
    Add-Content -Path $OutputFile -Value "$Message`n"
    Write-Host $Message
}
...
$Scenes = $Plot -split '\|'# Split the plot into an array of scenes
$PreviousScene = "First"  # Initialize the $PreviousScene variable
$PreviousChapter = 0 # Initialize the previous chapter number
foreach ($Scene in $Scenes) { # Iterate over each scene and process
    $ExpansionPrompt = ""  # Initialize $ExpansionPrompt for each iteration
    $Expansion = ""        # Initialize $Expansion for each iteration
    ...
    if ($PreviousScene -eq "First") { # Check if this is the first scene
