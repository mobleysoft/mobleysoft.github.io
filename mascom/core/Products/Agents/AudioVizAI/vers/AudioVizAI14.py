...
# Procedural pattern generator with dynamic visuals based on audio properties
def generate_pattern(scaled_frame_number, width, height, audio_dynamics):
    image = np.zeros((height, width, 3), dtype=np.uint8)
    dynamic_scale = audio_dynamics % 256

    for y in range(height):
        for x in range(width):
            r = (x ^ y ^ scaled_frame_number) % 256
            g = ((x & y) ^ (scaled_frame_number + dynamic_scale)) % 256
            b = (r * g + dynamic_scale) % 256
            image[y, x] = (b, g, r)
...
def generate_video(audio_file, output_file, width=320, height=240, upscale_factor=4, fps=30):
    ...
    audio_dynamics = int(np.mean(np.abs(y)) * 1000)  # Extract basic dynamics from audio

    for frame_number in tqdm(range(frame_count), desc="Generating frames"):
        scaled_frame_number = frame_number * 10
        pattern = generate_pattern(scaled_frame_number, width, height, audio_dynamics)
    ...
    if not os.path.exists(output_file):
        raise FileNotFoundError(f"Output video file {output_file} was not created.")

if __name__ == "__main__":
    input_dir = "inputs"
    output_dir = "outputs"
    used_dir = "used"
    historical_dir = "historicalOutputs"

    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(used_dir, exist_ok=True)
    os.makedirs(historical_dir, exist_ok=True)

    # Move existing outputs to historicalOutputs
    for existing_file in os.listdir(output_dir):
        existing_path = os.path.join(output_dir, existing_file)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        historical_path = os.path.join(historical_dir, f"{timestamp}_{existing_file}")
        os.rename(existing_path, historical_path)

    for audio_file in os.listdir(input_dir):
        if audio_file.endswith(".mp3"):
            input_path = os.path.join(input_dir, audio_file)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            artist_dir = os.path.join(output_dir, timestamp)
            os.makedirs(artist_dir, exist_ok=True)
            output_path = os.path.join(artist_dir, f"output_{timestamp}.mp4")
