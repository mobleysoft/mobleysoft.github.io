...
$SpeciesCount = 0
while (3 -lt $SpeciesCount) {
    $SpeciesCount++
    Write-Host "`n============================" -ForegroundColor Magenta
    Write-Host "EVOLVING NEW BOOK SPECIES #$SpeciesCount" -ForegroundColor Magenta
    Write-Host "============================`n" -ForegroundColor Magenta

    # Initialize word count tracking for this species
    $TotalWordCount = 0
    $TargetWordCount = 400000  # Each book cycle must exceed 400K words

    while ($TotalWordCount -lt $TargetWordCount) {
        $RunTimestamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $BookTitle = Invoke-OpenAI "Generate a new book title based on evolving themes." "AI"
        ...
        foreach ($capability in $Novelcapabilitys) {
            $SystemPrompt = "You are responsible for '$capability'. Generate new content for this book's evolution."
            $UserPrompt = "Write the next iteration of '$capability' that expands the overall library."

            # Invoke AI Call
            $capabilityContent = Invoke-OpenAI -Prompt $UserPrompt -SystemPrompt $SystemPrompt
            $BookContent += "`n### $capability`n$capabilityContent`n"
    ...
    Write-Host "`nSpecies #$SpeciesCount complete! Moving to next literary species...`n" -ForegroundColor Yellow
}
