...
$VideoPath = "C:\Books\Wordsmith"
if (-not (Test-Path $VideoPath)) { New-Item -ItemType Directory -Path $VideoPath | Out-Null }
# Video output path
$RunTimestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$VideoOutputPath = Join-Path $VideoPath "MobleysoftWordsmithDemo_$RunTimestamp.mp4"
...
function Start-Recording {
    ...
    Start-Process -NoNewWindow -FilePath $FFmpegPath -ArgumentList "-y -framerate 12 -f gdigrab -i desktop $OutputPath" -PassThru | Out-Null
...
Write-Host "Starting screen recording to capture the entire screen..." -ForegroundColor Green
